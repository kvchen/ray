...
int main(int argc, char** argv) {
  ...
  if (argc > 3) {
    ...
    if (log_file_name) {
      ...
      std::cout << "object store: writing to log file " << log_file_name << ", connected to redis at " << redis_host << ":" << redis_port << std::endl;
      create_log_dir_or_die(log_file_name);
      global_ray_config.log_to_file = true;
      global_ray_config.logfile.open(log_file_name);
      init_redis_log(global_ray_config, "objstore", argv[2], redis_host, std::stoi(redis_port));
    } else {
